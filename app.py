...
def get_alerts():
    ...
    try:
        if os.path.exists("data/alerts_log.csv"):
            ...
            return jsonify(
                {"alerts": alerts, "counts": severity_counts, "total": len(df)}
            )
        ...
        return jsonify(
            {
                ...
                "total": [
                    sum(x)
                    for x in zip(dos_counts, probe_counts, r2l_counts, u2r_counts)
                ],
            }
        )
            ...
            if not df.empty:
                ...
                if not attacks_df.empty:
                    ...
                    for ip, count in top_attackers.items():
                        ...
                        primary_risk = (
                            risk_levels.index[0] if not risk_levels.empty else "LOW"
                        )
                    ...
                    protocol_data.append(
                        {
                            ...
                            "bytes": count
                            * random.randint(500, 2000),  # Simulated bytes
                            "attacks": random.randint(
                                0, 20
                            ),  # Simulated attacks on this protocol
                        }
                    )
    ...
    try:
        ...
        if os.path.exists("data/predictions_log.csv"):
            ...
            if len(recent_df) > 0 and len(baseline_df) > 0:
                ...
                if baseline_rate > 0:
                    spike_percentage = (
                        (recent_rate - baseline_rate) / baseline_rate
                    ) * 100
    ...
    try:
        ...
        if os.path.exists("data/predictions_log.csv"):
            ...
            for _, row in paginated_df.iterrows():
                ...
                logs.append(
                    {
                        "id": hashlib.md5(str(row["timestamp"]).encode()).hexdigest()[
                            :8
                        ],
                        "timestamp": row["timestamp"].strftime("%Y-%m-%d %H:%M:%S"),
                        "source_ip": row["src_ip"],
                        "protocol": (
                            features_summary.split("|")[0].strip()
                            if features_summary
                            else "TCP"
                        ),
                        "service": (
                            features_summary.split("|")[1].strip()
                            if "|" in features_summary
                            else "http"
                        ),
                        "attack_type": row["attack_type"].upper(),
                        "confidence": (
        ...
        return jsonify(
            {
                ...
                "filters": {"all": 2, "attacks": 2, "critical": 1, "high": 1},
            }
        )
                ...
                "components": {
                    "attack_frequency": {
                        ...
                        "status": (
                            "good"
                            if attack_score >= 70
                            else "warning" if attack_score >= 50 else "critical"
                        ),
                    },
                    "risk_level": {
                        "score": risk_score,
...
def get_health_recommendations(overall, attack_score, risk_score):
    ...
    if overall < 60:
        recommendations.append(
            "🚨 Immediate action required - Security posture critical"
        )
    ...
    if risk_score < 50:
        recommendations.append(
            "⚠️ Multiple high-risk threats active - Prioritize investigation"
        )
        ...
        if os.path.exists("data/predictions_log.csv"):
            ...
            if len(hourly_attacks) > 24:
                ...
                if len(y) > 1:
                    ...
                    if np.std(y) > 0:
                        confidence = max(
                            50, min(95, 100 - (np.std(y) / np.mean(y) * 20))
                        )
                    else:
                        confidence = 85
                    ...
                    return jsonify(
                        {
                            ...
                            "peak_hour": (
                                next_hours[
                                    predicted_values.index(max(predicted_values))
                                ]
                                if predicted_values
                                else "N/A"
                            ),
    ...
    try:
        ...
        if os.path.exists("data/predictions_log.csv"):
            ...
            summary = {
                ...
                "Unique Attackers": (
                    df["src_ip"].nunique() if "src_ip" in df.columns else 0
                ),
            }
